...
def test_row_parallel_affine4_multilinear_matches_singleton(batch):
    ...
    linear = _bf16_quantization_parameters(
        MultiLinear(256, 128, 4).to_quantized(64, 4)
    )
    inputs = mx.random.normal((batch, 4, 1, 256)).astype(mx.bfloat16)
    expected_rows = [
        fast_ops.exact_affine_multilinear(
            linear, mx.contiguous(inputs[row : row + 1])
        )
...
def test_fused_fp8_gate_up_matches_native_selected_projections(
    ...
    expected = tuple(
        exact_quantized_switch_linear(linear, x, indices)
